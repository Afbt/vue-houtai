...
<script>
  ...
  export default {
    ...
    mounted() { //声明周期
      this.getcastlist();
      this.AsingleData(); //先调用下渲染页面
    },
    methods: {
      AsingleData() { //进入修改页面就要渲染的数据
        var url = '/admin/goods/getgoodsmodel/';
        var id = this.$route.params.ediTid;  //获取到通过get方式传的id参数
        ...
        this.$ajax.get(url+id)
          .then(res => {
            ...
            res.data.message.category_id = res.data.message.category_id-0;
      ...
      submitForm(formName) {
        this.$refs[formName].validate((valid) => {
          var url = '/admin/goods/edit/';
          var id = this.$route.params.ediTid;
          if (valid) {
            this.$ajax.post(url+id, this.ruleForm).then(res => {
              if (res.data.status == 1) {
                this.$notify.error({
                  title: '错误',
